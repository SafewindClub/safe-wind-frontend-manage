...
const close = (name: string) => {
  ...
  if (menuStore.activeName === name) {
    const nextTab = menuStore.tabs[index - 1];
    menuStore.activeName = nextTab.name;
    router.push({ name: menuStore.activeName });
  }
};
...
const handleDragEnd = (newTabs: any) => {
  // 更新标签顺序;
  const oldTab = menuStore.tabs[newTabs.oldIndex];
  menuStore.tabs[newTabs.oldIndex] = menuStore.tabs[newTabs.newIndex];
  menuStore.tabs[newTabs.newIndex] = oldTab;
};
</script>
